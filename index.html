      ...
      padding: 8px 15px;
        ...
        .foreign-body.grabbed {
          ...
          box-shadow: 
            ...
            0 0 30px rgba(0, 204, 102, 0.3);
          z-index: 32;
          animation: shake 0.3s infinite alternate;
        }
        
        @keyframes shake {
          0% { transform: translate(-50%, -50%) scale(1.2) rotate(-2deg); }
          100% { transform: translate(-50%, -50%) scale(1.2) rotate(2deg); }
        }
  ...
  <div id="container">
    ...
    <div id="alert-system">
      ...
      <div class="alert-container" id="vision-loss-alert">
        ...
        <div class="alert-content">
          ...
          <p>¡Precaución! Se ha aplicado láser en la mácula. Riesgo de pérdida visual permanente.</p>
          <div class="alert-timer"></div>
        </div>
        <button class="alert-dismiss">×</button>
      </div>
    ...
    <div id="eye-chamber">
      <div class="retina-container" id="retina">
        ...
        <!-- Cuerpo extraño -->
        <div id="foreign-body" class="foreign-body embedded"></div>
...
function updateInstrumentRod() {
  ...
  // Actualizar OCT para mostrar la pinza
  updateOCTScan();
}
...
function createForeignBody() {
  ...
  foreignBodySize = 40 + Math.random() * 20; // Tamaño entre 40-60px (aumentado para mejor visualización)
  ...
  foreignBody.classList.add('embedded');
...
function updateOCTScan() {
  ...
  // Capas de retina normales (sin distorsión)
  let retinaPath = "M0,40 Q75,30 150,40 T300,40";
  ...
  if (foreignBodyPresent) {
    ...
    if (activeInstrument === 'forceps') {
      const distance = Math.sqrt(Math.pow(tipRelX - relX, 2) + Math.pow(tipRelY - relY, 2));
      
      if (distance < relSize * 1.5) {
        // Efecto de pinza acercándose al objeto
        let forcepsPath = `M${tipRelX},${tipRelY} L${relX},${relY}`;
        
        // Si está agarrando, mostrar efecto de extracción con resistencia
        if (foreignBodyGrabbed) {
          if (!resistanceOvercome) {
            // Mostrar resistencia visual en OCT (el objeto se mueve menos que la pinza)
            const moveRatio = graspProgress / 200; // 0 a 1
            const midX = relX + (tipRelX - relX) * moveRatio * 0.5;
            const midY = relY + (tipRelY - relY) * moveRatio * 0.5;
            
            forcepsPath = `M${tipRelX},${tipRelY} L${midX},${midY}`;
            
            // Actualizar posición del objeto en OCT mientras hay resistencia
            const objPath = `
              M${relX-relSize/2},${relY-relSize/3} 
              Q${relX-relSize/4},${relY-relSize/2} ${relX},${relY-relSize/3}
              T${relX+relSize/2},${relY-relSize/3}
              ...
              T${relX-relSize/2},${relY-relSize/3}
              Z
              M${relX},${relY} L${midX},${midY}
            `;
            octForeignBody.setAttribute('d', objPath);
          } else {
            // Cuando se supera la resistencia, el objeto sigue completamente a la pinza
            forcepsPath = `M${tipRelX},${tipRelY} L${tipRelX},${tipRelY}`;
            octForeignBody.setAttribute('d', octPath + ` M${relX},${relY} L${tipRelX},${tipRelY}`);
          }
        }
...
function updateInstrumentPosition(normX, normY) {
  ...
  if (foreignBodyGrabbed) {
    ...
    // Aplicar resistencia al movimiento inicial
    if (!resistanceOvercome) {
      // El objeto se mueve menos que la pinza
      const moveRatio = graspProgress / 200; // 0 a 1
      const newX = foreignBodyX + (tipX - foreignBodyX) * moveRatio * 0.5;
      const newY = foreignBodyY + (tipY - foreignBodyY) * moveRatio * 0.5;
      
      foreignBody.style.left = `${newX}px`;
      foreignBody.style.top = `${newY}px`;
    } else {
      // Cuando se supera la resistencia, el objeto sigue completamente a la pinza
    ...
    // Verificar si está cerca del punto de entrada para extraerlo
    const distanceToBase = Math.sqrt(Math.pow(tipX - baseX, 2) + Math.pow(tipY - baseY, 2));
...
function checkForeignBodyGrasp() {
  ...
  if (distance < foreignBodySize * 1.2 && isActionButtonPressed) {
    if (!isGrasping) {
      // Comenzar a agarrar el cuerpo extraño
      isGrasping = true;
      graspProgress = 0;
      resistanceOvercome = false;
      ...
      foreignBody.classList.remove('embedded');
      tip.classList.add('grasping');
    }
    
    // Incrementar progreso de agarre
    graspProgress += 1;
    
    // Si se ha completado el agarre y superado la resistencia
    if (graspProgress >= 200 && !resistanceOvercome) {
      resistanceOvercome = true;
      
      // Efecto visual de que el objeto se libera
      createForeignBodyReleaseEffect();
      
      // Actualizar indicador
      const actionIndicator = document.getElementById('instrument-action-indicator');
      actionIndicator.textContent = "Cuerpo extraño agarrado";
    } else if (!resistanceOvercome) {
      // Mostrar progreso de superación de resistencia
      const actionIndicator = document.getElementById('instrument-action-indicator');
      actionIndicator.textContent = `Superando resistencia... ${Math.min(100, Math.round(graspProgress / 2))}%`;
    }
    
    foreignBodyGrabbed = true;
  } else if (isGrasping && !isActionButtonPressed) {
    // Si se soltó el botón antes de completar el agarre
    isGrasping = false;
    graspProgress = 0;
    resistanceOvercome = false;
...
function createForeignBodyReleaseEffect() {
  const foreignBody = document.getElementById('foreign-body');
  
  // Crear partículas de polvo/metal al liberar el cuerpo extraño
  for (let i = 0; i < 5; i++) {
    const particle = document.createElement('div');
...
function startAction() {
  ...
  vitrectomyInterval = setInterval(() => {
    ...
    if (activeInstrument === 'vitrectomo') {
      ...
      if (!resistanceOvercome) {
        const moveRatio = graspProgress / 200; // 0 a 1
        const newX = foreignBodyX + (tipRect.left - retinaRect.left + tipRect.width/2 - foreignBodyX) * moveRatio * 0.5;
        const newY = foreignBodyY + (tipRect.top - retinaRect.top + tipRect.height/2 - foreignBodyY) * moveRatio * 0.5;
      ...
      const distanceToBase = Math.sqrt(
        Math.pow(tipRect.left + tipRect.width/2 - baseRect.left - baseRect.width/2, 2) + 
        Math.pow(tipRect.top + tipRect.height/2 - baseRect.top - baseRect.height/2, 2)
      );
...
function stopAction() {
  ...
  if (isGrasping && !foreignBodyGrabbed) {
    releaseForeignBody();
  }
}
